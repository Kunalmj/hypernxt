            ...
            <div key={t.id} className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 hover:shadow-md transition-shadow flex flex-col md:flex-row flex-wrap gap-6 md:items-center">
              ...
              <div className="flex-1 space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="px-3 py-1 bg-red-50 text-red-700 rounded-full text-xs font-bold uppercase tracking-wider border border-red-100">
                    📅 {t.deadline}
                  </span>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  {t.tags?.map((tag) => (
                    <span key={tag} className="px-2.5 py-1 bg-purple-50 text-purple-700 rounded text-xs font-medium border border-purple-100">{tag}</span>
                  ))}
                </div>
              </div>
                ...
                <div className="w-full border-t border-slate-100 pt-6 mt-2 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                    <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                      <p className="text-xs font-semibold text-emerald-500 uppercase tracking-wider mb-1">Contract Value</p>
                      <p className="text-base font-bold text-emerald-700">{t.value}</p>
                    </div>
                    <div className="bg-red-50 rounded-xl p-4 border border-red-100">
                      <p className="text-xs font-semibold text-red-400 uppercase tracking-wider mb-1">Bid Deadline</p>
                      <p className="text-base font-bold text-red-600">{t.deadline}</p>
                    </div>
                    <div className="bg-amber-50 rounded-xl p-4 border border-amber-100">
                      <p className="text-xs font-semibold text-amber-500 uppercase tracking-wider mb-1">EMD Amount</p>
                      <p className="text-base font-bold text-amber-700">{t.emd}</p>
                    </div>
                    <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
                      <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Bid Mode</p>
                      <p className="text-sm font-bold text-purple-700">{t.bidSubmissionMode?.split(" (")[0] || "Online"}</p>
                    </div>
                  </div>
                      ...
                      <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-semibold text-red-600">{t.importantDates.lastDateSubmission}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-400 font-medium">Bid Opening</p>
                            <p className="text-sm font-semibold text-emerald-600">{t.importantDates.openingDate}</p>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="mt-5 flex flex-col sm:flex-row items-center gap-3 justify-between bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-blue-700">{t.website || "Official tender portal"}</span>
                    </div>
                    <button
                      onClick={() => navigate("/tender-apply", { state: { tender: t } })}
