      ...
      <div className="min-h-screen bg-gradient-to-b from-[#f0f4ff] to-white flex items-center justify-center p-6 font-sans">
        <div className="bg-white rounded-3xl shadow-xl border border-blue-100 max-w-lg w-full p-10 text-center">
          ...
          <p className="text-slate-500 mb-2 text-base">
            Your application for <span className="font-semibold text-blue-700">{program?.title || "the program"}</span> has been submitted successfully.
          </p>
          <p className="text-sm text-slate-400 mb-8">Application ID: <span className="font-mono font-semibold text-slate-600">WMN-{Date.now().toString().slice(-8)}</span></p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => navigate("/women-results", { state: { results: [] } })} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-colors shadow-sm">
              Back to Results
            </button>
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f0f4ff] to-white font-sans pb-20">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white pt-10 pb-14 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2">Women Program Application</h1>
          {program && (
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-6 mt-8">
        ...
        {activeSection === 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Full Name" required value={formData.fullName} onChange={(v) => update("fullName", v)} placeholder="e.g. Priya Sharma" />
              ...
              <Field label="Email Address" type="email" value={formData.email} onChange={(v) => update("email", v)} placeholder="priya@example.com" />
              <Field label="Date of Birth" type="date" required value={formData.dob} onChange={(v) => update("dob", v)} />
              <SelectField label="Marital Status" required value={formData.maritalStatus} onChange={(v) => update("maritalStatus", v)} options={["Single", "Married", "Widowed", "Divorced", "Other"]} />
            </div>
        ...
        {activeSection === 1 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6 animate-fadeIn">
            <div>
              <h2 className="text-xl font-bold text-slate-800 mb-1">Background & Location</h2>
              <p className="text-sm text-slate-500">Details about your education and residence.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                <SelectField label="Highest Education Completed" required value={formData.education} onChange={(v) => update("education", v)} options={["Below 10th", "10th Pass", "12th Pass", "Graduate", "Post Graduate", "Professional Degree"]} />
              </div>
              <div className="md:col-span-2">
                <Field label="Residential Address" required value={formData.address} onChange={(v) => update("address", v)} placeholder="House No, Street, Locality" />
              </div>
              <Field label="City / Town" required value={formData.city} onChange={(v) => update("city", v)} placeholder="e.g. Pune" />
              <Field label="State" required value={formData.state} onChange={(v) => update("state", v)} placeholder="e.g. Maharashtra" />
            </div>
        ...
        {activeSection === 2 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6 animate-fadeIn">
            <div>
              ...
              <p className="text-sm text-slate-500">Provide bank details for any financial support or stipends.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {activeSection === 3 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6 animate-fadeIn">
            <div>
              <h2 className="text-xl font-bold text-slate-800 mb-1">Statement & Documents</h2>
              <p className="text-sm text-slate-500">Upload required proofs and tell us why you need this program.</p>
            </div>
            
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                Why do you need this support? <span className="text-red-500">*</span>
              </label>
              <textarea
                value={formData.briefNeed}
                onChange={(e) => update("briefNeed", e.target.value)}
                rows={4}
                ...
                placeholder="Briefly describe your situation and how this program will help you..."
                required
              />
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {["Aadhaar Card", "Education Proof", "Bank Passbook Front", "Passport Photo"].map((doc) => (
            ...
            <div className="bg-blue-50 rounded-xl p-5 border border-blue-100 mt-4">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-slate-700 leading-relaxed">
                  I hereby declare that all information provided is true and correct to the best of my knowledge. I agree to the <span className="text-blue-700 font-medium">Terms & Conditions</span>.
                </span>
              </label>
            </div>
