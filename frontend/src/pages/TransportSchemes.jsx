...
const BENEFICIARY_TYPES = ["All Beneficiaries", "EV Buyers", "General Public", "Transporters"];
  ...
  <div className="mb-5">
    ...
    <div className="relative">
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full appearance-none bg-white border border-[#e2e8f0] rounded-xl px-3 py-2.5 text-[0.85rem] text-[#0f172a] font-medium cursor-pointer focus:outline-none focus:border-[#0891b2]">
...
const TransportSchemes = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb]">
      <div className="bg-gradient-to-r from-[#0891b2] to-[#22d3ee] px-6 md:px-12 py-8">
        <div className="max-w-6xl mx-auto">
          <button onClick={() => navigate(-1)} className="inline-flex items-center gap-1.5 text-cyan-100 text-[0.82rem] font-semibold bg-transparent border-none cursor-pointer p-0 mb-4 hover:text-white transition-colors">← Back to All Categories</button>
          <h1 className="text-white text-[1.45rem] md:text-[1.75rem] font-extrabold mb-1">Transport &amp; Infrastructure</h1>
          <p className="text-cyan-100 text-[0.85rem]">Explore 90 schemes in this category</p>
          <div className="mt-5 relative max-w-2xl">
            ...
            <input type="text" placeholder="Search schemes..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-xl border-none text-[0.9rem] text-[#0f172a] placeholder-[#94a3b8] focus:outline-none focus:ring-2 focus:ring-cyan-300" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.12)" }} />
          </div>
        </div>
      </div>
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 flex gap-6 items-start">
        {showFilters && (
          <aside className="w-64 flex-shrink-0 bg-white rounded-2xl p-5 sticky top-[96px]" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06), 0 4px 12px rgba(37,99,235,0.06), inset 0 0 0 1px rgba(226,232,240,0.8)" }}>
            <div className="flex items-center gap-2 mb-5"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0891b2" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" /></svg><span className="font-bold text-[#0f172a] text-[0.9rem]">Filters</span></div>
            ...
            {filtered.length === 0 ? (<div className="bg-white rounded-2xl p-10 text-center" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06), inset 0 0 0 1px rgba(226,232,240,0.8)" }}><p className="text-[#64748b] text-[0.9rem]">No schemes match your filters.</p></div>) : (
              filtered.map((scheme) => (
                <div key={scheme.id} className="bg-white rounded-2xl p-6 flex gap-4 transition-all duration-200" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.05), 0 4px 12px rgba(37,99,235,0.06), inset 0 0 0 1px rgba(226,232,240,0.8)" }}>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[1rem] font-bold text-[#0891b2] mb-0.5 leading-snug cursor-pointer hover:underline">{scheme.title}</h3>
                  ...
                  <div className="flex flex-col justify-between items-end flex-shrink-0 min-w-[160px] gap-3">
                    <div className="flex flex-col gap-2 text-right w-full">
                      <div className="bg-[#ecfeff] border border-[#a5f3fc] rounded-xl px-3 py-2"><p className="text-[0.68rem] text-[#0891b2] font-bold uppercase tracking-wide mb-0.5">Benefit</p><p className="text-[0.82rem] font-semibold text-[#0f172a]">{scheme.benefit}</p></div>
                    </div>
                    <div className="flex flex-col gap-2 w-full">
                      <button className="w-full inline-flex items-center justify-center gap-1.5 bg-[#0891b2] text-white text-[0.8rem] font-bold rounded-xl px-4 py-2.5 border-none cursor-pointer hover:bg-[#0e7490] transition-colors">View Details →</button>
                    </div>
                  </div>
                </div>
              ))
